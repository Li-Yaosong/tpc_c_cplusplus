...
- 将zxing-cpp拷贝至tools/main目录下
  ```
  cd tpc_c_cplusplus
  cp thirdparty/zxing-cpp tools/main -rf
  ```
- 在tools目录下编译三方库
...
- 三方库头文件及生成的库
  ...
  ```
  zxing-cpp-arm64-v8a-install   zxing-cpp-armeabi-v7a-install
  ```
...
&nbsp;![thirdparty_install_dir](pic/zxing-cpp_install_dir.jpg)
- 在最外层（cpp目录下）CMakeLists.txt中添加如下语句
  ```
  #将三方库加入工程中
  add_library(zxing SHARED IMPORTED)
  set_target_properties(zxing
        PROPERTIES IMPORTED_LOCATION
        ${CMAKE_SOURCE_DIR}/../../../libs/${OHOS_ARCH}/libZXing.so)
  target_link_libraries(entry PUBLIC libace_napi.z.so zxing)   
  #将三方库的头文件加入工程中
  target_include_directories(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/zxing-cpp-${OHOS_ARCH}-install/include)
  ```
  ![zxing-cpp_usage](pic/zxing-cpp_usage.jpg)
## 测试三方库
...
- 将准备好的文件推送到开发板，在windows命令行进行如下操作
  ...
  hdc_std file send libZXing.so /system/lib64
...
&nbsp;![zxing-cpp_test](pic/zxing-cpp_test.jpg)
