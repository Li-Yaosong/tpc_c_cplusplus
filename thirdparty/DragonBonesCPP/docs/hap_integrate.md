...
```
#将三方库加入工程中
target_include_directories(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/libmad/${OHOS_ARCH}/lib/libDragonBones.a)
#将三方库的头文件加入工程中
target_include_directories(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/libmad/${OHOS_ARCH}/include)
```
  ![thirdparty_usage](pic/usage.png)
## 测试三方库
三方库的测试使用原库自带的测试用例来做测试

进入到构建目录获取,执行ctest，运行测试用例（DragonBonesCPP-arm64-v8a-build为构建64位的目录，DragonBonesCPP-armeabi-v7a-build为构建32位的目录）
