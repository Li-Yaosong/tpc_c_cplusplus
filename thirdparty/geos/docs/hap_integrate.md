...
- 三方库目录结构
  ```
  tpc_c_cplusplus/thirdparty/geos  #三方库geos的目录结构如下
  ├── docs                              #三方库相关文档的文件夹
  ├── HPKBUILD                          #构建脚本
  ├── SHA512SUM                         #三方库校验文件
...
- 将geos拷贝至tools/main目录下
  ```
  cd tpc_c_cplusplus
  cp thirdparty/geos tools/main -rf
  ```
- 在tools目录下编译三方库
...
- 三方库头文件及生成的库
  ...
  ```
  geos-arm64-v8a-install   geos-armeabi-v7a-install
  ```
...
&nbsp;![thirdparty_install_dir](pic/geos_install_dir.jpg)
- 在最外层（cpp目录下）CMakeLists.txt中添加如下语句
  ...
  set_target_properties(geos
        ...
        ${CMAKE_SOURCE_DIR}/../../../libs/${OHOS_ARCH}/libgeos.so)
  add_library(geos_c SHARED IMPORTED)
  set_target_properties(geos_c
        PROPERTIES IMPORTED_LOCATION
        ${CMAKE_SOURCE_DIR}/../../../libs/${OHOS_ARCH}/libgeos_c.so)        
  target_link_libraries(entry PUBLIC libace_napi.z.so geos geos_c)   
  #将三方库的头文件加入工程中
  target_include_directories(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/geos-${OHOS_ARCH}-install/include)
  ```
  ![geos_usage](pic/geos_usage.jpg)
## 测试三方库
