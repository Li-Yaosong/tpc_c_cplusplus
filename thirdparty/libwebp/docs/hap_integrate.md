...
- 将三方库拷贝至tools/main目录下
  ...
  cp thirdparty/xxHash  tools/main/ -rf
...
- 在tools目录下编译三方库
  ...
  ```
  cd tools
  ./build.sh ./build.sh xxHash libpng libjpeg-turbo 
  ```
- 三方库头文件及生成的库
...
target_link_libraries(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/libwebp/${OHOS_ARCH}/lib/libsharpyuv.so.0.0.0)
target_link_libraries(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/libwebp/${OHOS_ARCH}/lib/libwebp.so.7.1.6)
target_link_libraries(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/libwebp/${OHOS_ARCH}/lib/libwebpdecoder.so.3.1.6)
target_link_libraries(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/libwebp/${OHOS_ARCH}/lib/libwebpdemux.so.2.0.12)
target_link_libraries(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/libwebp/${OHOS_ARCH}/lib/libwebpmux.so.3.0.11)
#将三方库的头文件加入工程中
