...
进入到构建目录data/tpc_c_cplusplus/thirdparty/lycium，执行./test.sh mupdf 运行测试用例（arm64-v8a-build为构建64位的目录，armeabi-v7a-build为构建32位的目录）然后在/data/tpc_c_cplusplus/lycium/usr/mupdf/armeabi-v7a/bin路径下查看相应的pdf文件以及图片是否生成
测试用例结果如下：&nbsp;![thirdparty_install_dir](pic/test_result1.png)

![thirdparty_install_dir](pic/test_result2.png)
