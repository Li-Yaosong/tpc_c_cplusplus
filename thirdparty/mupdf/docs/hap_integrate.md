...
首先将交叉编译生成的库文件放到新建的NAPI工程中（参考[通过DevEco Studio开发一个NAPI工程](https://gitee.com/openharmony-sig/knowledge_demo_temp/blob/master/docs/napi_study/docs/hello_napi.md)），然后将mupdf提供的三个测试demo放到工程中编译引用，分别测试生成pdf能力，读取pdf能力以及多线程能力。
测试用例结果如下：&nbsp;![thirdparty_install_dir](pic/test_mupdf.png)
