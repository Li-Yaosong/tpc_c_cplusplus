...
- 三方库目录结构
  ...
  ```shell
  tpc_c_cplusplus/thirdparty/md5-c  #三方库md5-c的目录结构如下
  ├── docs                              #三方库相关文档的文件夹
  ├── HPKBUILD                          #构建脚本
  ├── md5-c_oh_pkg.patch                #补丁
  ├── OAT.xml                           #xml文件
  ├── SHA512SUM                         #三方库校验文件
...
- 将md5-c拷贝至tools/main目录下

  ```shell
  cd tpc_c_cplusplus
  cp thirdparty/md5-c tools/main -rf
  ```
...
- 三方库头文件及生成的库
  ...
  ```shell
  md5-c/arm64-v8a   md5-c/armeabi-v7a
  ```
- [测试三方库](#测试三方库)
...
- 在IDE的cpp目录下新增thirdparty目录，将编译生成的库拷贝到该目录下，如下图所示
  ...
  ![thirdparty_install_dir](pic/md5-c_install_dir.png)

- 在最外层（cpp目录下）CMakeLists.txt中添加如下语句

  ```shell
  #将三方库加入工程中
  target_link_libraries(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/md5-c/${OHOS_ARCH}/lib/libmd5.a)

  #将三方库的头文件加入工程中
  target_include_directories(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/md5-c/${OHOS_ARCH}/include)
  ```

  ![md5-c_usage](pic/md5-c_usage.png)
