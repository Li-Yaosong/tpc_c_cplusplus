...
- 三方库目录结构
  ...
  ├── README_zh.md   
  ```
- 将libsvm拷贝至工程xxxx/entry/src/main/cpp/thirdparty目录下
### 准备三方库源码
- 三方库下载地址：[libsvm](https://github.com/cjlin1/libsvm), 版本：master
  解压后修改库文件名为libsvm，拷贝至工程xxxx/entry/src/main/cpp/thirdparty/libsvm目录下 
## 应用中使用三方库
- 将三方库加入工程中，目录结构如下
  ```
...
- 在工程顶级CMakeLists.txt中引入三方库，增加如下代码
  ```
  add_subdirectory(thirdparty/libsvm)      #引入子目录下的CMakeLists.txt
  target_link_libraries(工程库名 PUBLIC libsvm)   #工程依赖三方库libsvm
  target_include_directories(工程库名 PRIVATE thirdparty/libsvm)
  ```
## 编译工程
...
## 运行效果
- 在 [libsvm](https://gitee.com/openharmony-tpc/openharmony_tpc_samples/tree/master/libsvm)中，运行效果如下图
  &nbsp;![演示](pic/hap.jpg)
## 参考资料
